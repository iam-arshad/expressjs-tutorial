...
jest.mock('../../src/database/schemas/users', () => {
    const testUser = {
        id: 'gfh442yg',//this is in string which needs to be converted to mongodb's ObjectId type.But in jest.mock() we cant use other modules like mongoose.Need to create a jest setupfile for this.
...
describe('Able to access all protected routes, after logging in', () => {
    ...
    beforeAll(async () => {
            const response = await request(app)
                .post('/auth/login')
                .send({ username: "abc@abc.com", password: "abc@abc.com" });
            
            sessionId = response.headers['set-cookie'];
            responseBody = response.body;
    }, 20000); // Increase timeout to 20 seconds just in case
